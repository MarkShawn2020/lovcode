...
- a027ae8: 统一关键词检索与结果高亮的布尔、短语和完整标识符匹配规则，确保命中证据出现在结果摘要中；同时完善档案项目标签、相对时间设置、项目与来源筛选，以及大型下拉框的输入筛选体验。
...
- Persist the search overlay view mode and keep recent search chips on a single row.
